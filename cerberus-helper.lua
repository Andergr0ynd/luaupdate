...
script_version("19.09.2024.17.12")
...
return {
    check=function (a,b,c)
        ...
        downloadUrlToFile(a, e, function(g, h, i, j)
            if h==d.STATUSEX_ENDDOWNLOAD then
                if doesFileExist(e) then
                    ...
                    if k then
                        ...
                        if updateversion ~= thisScript().version then
                            lua_thread.create(function(b)
                                local d=require('moonloader').download_status
                                local m=-1
                                sampAddChatMessage(b..'Обновление завершено. Пожалуйста обновитесь с '..thisScript().version..' на '..updateversion, m)
                                wait(250)
                                
                                downloadUrlToFile(updatelink, thisScript().path, function(n, o, p, q)
                                    if o==d.STATUS_DOWNLOADINGDATA then
                                        print(string.format('Загружается %d из %d.', p, q))
                                    ...
                                    if o==d.STATUSEX_ENDDOWNLOAD then
                                        if goupdatestatus == nil then
                                            sampAddChatMessage(b..'Обновление прошло неудачно. Пожалуйста ознакомьтесь с...', m)
                                            update = false
                                        end
                                    end
                                end)
                            ...
                            update = false
                            print('v'..thisScript().version..': Обновление не доступно.')
                        end
                    ...
                    update = false
                    print('v'..thisScript().version..': Не могу проводить обновление. Свяжитесь или проведите с нашим администратором на '..c)
                end
            end
        end)
        ...
        if os.clock() - f >= 10 then
            print('v'..thisScript().version..': timeout, выйдите из учета обновления. Свяжитесь или проведите с нашим администратором на '..c)
        end
    end
}
]]])
